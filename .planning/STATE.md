...
- 2026-04-27: Phase 10 added — **Orphan Detection Precision Refactor**. Goal: refondre le moteur de détection des rémanents AppData (>90 % de FP mesurés sur corpus de 365 items dans `C:\ProgramData`) pour atteindre <5 % de FP sans dégrader le rappel. Pipeline 7 étapes : HardBlacklist → ParentContextAnalyzer → KnownPathRules → MultiSourceMatcher (Service/Driver/Appx/Process/ScheduledTask) → PublisherAliasResolver → ConfidenceScorer → RiskLevelClassifier. Nouveau modèle `AppDataOrphanCandidate` (ConfidenceScore 0-100, RiskLevel, RecommendedAction, traçabilité des règles déclenchées). Décisions verrouillées : (a) règles JSON cohérentes avec `Resources/PublisherRules`, (b) nouveau `PathRuleEngine` parallèle au `PublisherRuleEngine`, (c) promotion `IServiceEnumerator`/`IScheduledTaskEnumerator` de internal à public + ajout `IDriverEnumerator`/`IAppxEnumerator`, (d) corpus fixture 365 items, (e) `Package Cache\*` plafonné à `VerifierAvant`, (f) `MachineSnapshot` lazy + TTL 5 min. 6 plans prévus (10-01 à 10-06). Branche : `feature/10-orphan-detection-precision`.
